 ...
 *
 * Idempotent: re-running wipes any previously seeded @clinic-demo.com rows
...
const DEMO_DOMAIN = '@clinic-demo.com';
...
function blocksOn(doctorIndex: number, dateStr: string) {
  const out: Array<{ startTime: string; endTime: string } | { startTime: null; endTime: null }> = [];
  if (FULL_DAY_FOR[doctorIndex] === dateStr) out.push({ startTime: null, endTime: null });
  if (PARTIAL_FOR[doctorIndex] === dateStr) out.push({ startTime: '12:00', endTime: '14:00' });
  return out;
}

/** Slot starts (UTC) for a doctor on a given date, respecting that date's blocks. */
function slotsForDay(doctor: DoctorDef, dateStr: string, dayOfWeek: number): string[] {
    ...
    while (t < end) {
      ...
      if (tEnd <= end) {
        const blocked = dayBlocks.some(
          (b) => !b.startTime || (t < toMin(b.endTime!) && tEnd > toMin(b.startTime!)),
        );
...
async function run() {
  ...
  await db.transaction(async (tx) => {
    ...
    if (ids.length > 0) {
      await tx.delete(schema.notifications).where(inArray(schema.notifications.userId, ids));
      await tx.delete(schema.appointments).where(
        or(inArray(schema.appointments.doctorId, ids), inArray(schema.appointments.patientId, ids)),
      );
      await tx.delete(schema.waitingList).where(
        or(inArray(schema.waitingList.doctorId, ids), inArray(schema.waitingList.patientId, ids)),
      );
    ...
    for (let i = 0; i < doctorRows.length; i++) {
      ...
      await tx.insert(schema.blockedSlots).values([
        { doctorId: doctorRows[i]!.id, blockDate: fullDay, startTime: null, endTime: null },
        { doctorId: doctorRows[i]!.id, blockDate: partial, startTime: '12:00', endTime: '14:00' },
      ]);
    }
    ...
    for (let di = 0; di < doctorRows.length; di++) {
      ...
      for (const dateStr of DATES) {
        ...
        starts.forEach((start, idx) => {
          ...
          const status: 'completed' | 'cancelled' | 'scheduled' =
            isPast ? (idx % 7 === 3 ? 'cancelled' : 'completed') : idx % 11 === 5 ? 'cancelled' : 'scheduled';
          patientIdx++;
          appointmentsToInsert.push({
            ...
            status,
            createdAt: new Date(startMs - (1 + (idx % 3)) * 86_400_000 + (idx % 12) * 3600_000),
            ...(status === 'cancelled' ? { cancelledAt: new Date(Math.min(now, startMs) - 3600_000) } : {}),
          });
        });
      }
    }
    ...
    const target = await tx
      ...
      .where(and(eq(schema.appointments.doctorId, doctorRows[0]!.id), eq(schema.appointments.status, 'scheduled')))
      .limit(1);
    if (target[0]) {
      const slot = target[0];
      const queued = await tx
        ...
        .returning({ id: schema.waitingList.id, patientId: schema.waitingList.patientId });
      ...
      await tx.insert(schema.notifications).values({
        ...
        type: 'waitlist_offer',
        payload: { doctorId: slot.doctorId, slotStart: slot.startTime.toISOString(), offerExpiresAt: offerExpiresAt.toISOString() },
      ...
      await tx.insert(schema.notifications).values({
        ...
        payload: { doctorId: slot.doctorId, slotStart: slot.startTime.toISOString() },
        sentAt: new Date(nowIso),
      });
      ...
      await tx
        .update(schema.waitingList)
        .set({ status: 'expired', offeredAt: new Date(now - 30 * 60_000), offerExpiresAt: new Date(now - 15 * 60_000) })
    ...
    const upcoming = await tx
      .select()
      .from(schema.appointments)
      .where(and(eq(schema.appointments.doctorId, doctorRows[1]!.id), eq(schema.appointments.status, 'scheduled')))
    ...
    for (const a of upcoming) {
      await tx.insert(schema.notifications).values({
        userId: a.patientId,
        type: 'reminder',
        payload: { doctorId: a.doctorId, appointmentId: a.id, startTime: a.startTime.toISOString() },
        sentAt: new Date(nowIso),
      });
    }
    ...
    console.log(`  doctors      = ${doctorRows.map((d) => `${d.id}:${d.email}`).join(', ')}`);
    console.log(`  patients     = ${patientRows.length} (${patientRows.map((p) => p.email).join(', ')})`);
    console.log(`  analytics    = GET /doctors/:id/analytics?month=${DEMO_MONTH}`);
